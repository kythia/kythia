/**
 * @namespace: addons/ai/helpers/factsUi.js
 * @type: Helper Script
...
async function generateFactsContainer(
	...
	if (pageFacts.length === 0) {
		listText = await t(interaction, 'ai.helpers.factsUi.ai.facts.empty');
	} else {
	...
	const [factsContainer] = await createPaginationContainer(interaction, {
		page,
		totalPages,
		title: await t(interaction, 'ai.helpers.factsUi.ai.facts.title'),
		content: listText,
		footer: await t(interaction, 'ai.helpers.factsUi.ai.facts.footer', {
			page,
			totalPages,
			totalFacts,
		}),
