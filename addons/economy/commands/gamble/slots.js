/**
 * @namespace: addons/economy/commands/gamble/slots.js
 * @type: Command
...
class SlotsCommand extends BaseCommand {
	...
	async execute(interaction) {
		...
		if (user.kythiaCoin < bet) {
			const msg = await t(
				interaction,
				'economy.commands.gamble.slots.slots.not.enough.cash',
				{
		...
		const spinningMsg = await t(
			interaction,
			'economy.commands.gamble.slots.slots.spinning.desc',
		);
		...
		if (r1.emoji === r2.emoji && r2.emoji === r3.emoji) {
			...
			resultTitle = await t(
				interaction,
				'economy.commands.gamble.slots.slots.jackpot.title',
			);
			resultColor = 'Gold';
		} else if (
			r1.emoji === r2.emoji ||
			...
			resultTitle = await t(
				interaction,
				'economy.commands.gamble.slots.slots.bigwin.title',
			);
			resultColor = 'Green';
			...
			resultTitle = await t(
				interaction,
				'economy.commands.gamble.slots.slots.lucky.title',
			);
			resultColor = 'Blue';
		} else {
			resultTitle = await t(
				interaction,
				'economy.commands.gamble.slots.slots.lose.title',
			);
		}
		if (winnings > 0) {
		...
		const resultMsg = [
			...
			slotDisplay,
			'',
			`**${await t(interaction, 'economy.commands.gamble.slots.slots.bet.field')}:** 🪙 ${bet.toLocaleString()}`,
			`**${await t(interaction, 'economy.commands.gamble.slots.slots.win.field')}:** 🪙 ${winnings.toLocaleString()} (${payoutMultiplier}x)`,
			`**${await t(interaction, 'economy.commands.gamble.slots.slots.cash.field')}:** 💰 ${user.kythiaCoin.toLocaleString()}`,
		].join('\n');
