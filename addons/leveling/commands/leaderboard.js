...
class LeaderboardCommand extends BaseCommand {
	...
	async execute(interaction) {
		...
		if (topUsers.length === 0) {
			leaderboard = await t(
				interaction,
				'leveling.commands.leaderboard.leveling.leaderboard.empty',
			);
		} else {
			leaderboard = (
				await Promise.all(
					topUsers.map(
						async (user, index) =>
							await t(
								interaction,
								'leveling.commands.leaderboard.leveling.leaderboard.entry',
								{
									rank: index + 1,
									userId: user.userId,
									level: user.level || 0,
		...
		const components = await simpleContainer(
			interaction,
			`${await t(interaction, 'leveling.commands.leaderboard.leveling.leaderboard.title')}\n${leaderboard}`,
			{
