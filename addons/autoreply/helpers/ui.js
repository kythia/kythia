/**
 * @namespace: addons/autoreply/helpers/ui.js
 * @type: Helper Script
...
async function generateListContainer(
	...
	if (pageItems.length === 0) {
		content = await t(interaction, 'autoreply.helpers.ui.list.empty');
	} else {
		const entries = await Promise.all(
			pageItems.map(async (reply) => {
				const containerTag = reply.useContainer
					? await t(interaction, 'autoreply.helpers.ui.list.container_tag')
					: '';
				return t(interaction, 'autoreply.helpers.ui.list.entry', {
	...
	const [listContainer] = await createPaginationContainer(interaction, {
		page,
		totalPages,
		title: await t(interaction, 'autoreply.helpers.ui.list.title', {
			page,
			totalPages,
		}),
		content,
		footer: await t(interaction, 'autoreply.helpers.ui.list.footer', {
			page,
			totalPages,
		}),
