	...
	async execute(interaction, container) {
		...
		if (subcommand === 'setup') {
			...
			await interaction.reply({
				content:
					'Klik tombol di bawah untuk memberikan izin Kythia memperbarui widget di profilmu ya! 😉\n\n*-# Tenang aja, Kythia nggak akan simpan token kamu kok.*',
				components: [row],
				ephemeral: true,
			});
			return;
		...
		if (subcommand === 'refresh') {
			...
			const payload = {
				...
				data: {
					dynamic: [
						{ type: 1, name: 'kythia_coin', value: String(Number(kythiaCoin)) },
						{ type: 1, name: 'kythia_ruby', value: String(Number(kythiaRuby)) },
						{ type: 1, name: 'vote_points', value: String(Number(votePoints)) },
						{
							type: 1,
