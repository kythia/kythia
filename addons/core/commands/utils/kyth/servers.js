	...
	async execute(interaction) {
		...
		if (totalGuilds === 0) {
			const { containerStr } = await container.helpers.core[
				'servers-ui'
			].generateServersContainer(interaction, 1, [], 0, true);
		...
		const { containerStr: initContainer, totalPages: initPages } =
			await container.helpers.core['servers-ui'].generateServersContainer(
				interaction,
				currentPage,
				guilds,
				totalGuilds,
		...
		collector.on('collect', async (i) => {
			...
			const { containerStr: newContainer } = await container.helpers.core[
				'servers-ui'
			].generateServersContainer(i, currentPage, guilds, totalGuilds);
