...
class DebugCommand extends BaseCommand {
	...
	slashCommand = (subcommand) =>
		subcommand
			.setName('debug-cache')
			.setDescription(
				'[DEV] Run comprehensive diagnostic tests on all KythiaModel caching methods.',
			);
	async execute(interaction) {
		...
		try {
			...
			const { res: delTracks, time: tDelT } = await t(() =>
				PlaylistTrack.destroyAndClearCache({
					where: {
						playlistId,
					},
				}),
