/**
 * @namespace: addons/core/helpers/serversUi.js
 * @type: Helper Script
...
async function generateServersContainer(
	...
	if (pageGuilds.length === 0) {
		contentText = await t(
			interaction,
			'core.helpers.serversUi.utils.kyth.servers.empty',
		);
	} else {
		const entries = await Promise.all(
			pageGuilds.map(async (guild, index) => {
				...
				return await t(
					interaction,
					'core.helpers.serversUi.utils.kyth.servers.entry',
					{
						rank,
						name: guild.name,
						id: guild.id,
	...
	const [containerStr] = await createPaginationContainer(interaction, {
		...
		title: await t(
			interaction,
			'core.helpers.serversUi.utils.kyth.servers.title',
		),
		content: contentText,
		footer: await t(
			interaction,
			'core.helpers.serversUi.utils.kyth.servers.footer',
			{
				page,
				totalPages,
