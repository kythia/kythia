...
async function generatePremiumListContainer(
	...
	if (pageUsers.length === 0) {
		listText = await t(
			interaction,
			'core.helpers.premiumListUi.premium.premium.list.empty',
		);
	} else {
		const entries = await Promise.all(
			pageUsers.map(async (p, index) => {
				...
				return await t(
					interaction,
					'core.helpers.premiumListUi.premium.premium.list.item',
					{
						index: globalIndex,
						user: `<@${p.userId}>`,
	...
	const [premiumListContainer] = await createPaginationContainer(interaction, {
		...
		title: await t(
			interaction,
			'core.helpers.premiumListUi.premium.premium.list.title',
		),
		content: listText,
		footer: await t(
			interaction,
			'core.helpers.premiumListUi.premium.premium.list.footer',
			{
				page,
				totalPages,
				totalUsers,
