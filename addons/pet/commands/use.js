	...
	async execute(interaction) {
		...
		if (!userPet) {
			const components = await simpleContainer(
				interaction,
				await t(interaction, 'pet.commands.use.no.pet.title_md'),
				{
					color: 'Red',
				},
			);
		...
		if (justDied) {
			try {
				await interaction.user.send(
					await t(interaction, 'pet.commands.use.dead.dm'),
				);
			} catch (_e) {
			...
			const components = await simpleContainer(
				interaction,
				await t(interaction, 'pet.commands.use.dead.title_md'),
				{
					color: 'Red',
				},
			);
		...
		if (cooldown.remaining) {
			const components = await simpleContainer(
				interaction,
				await t(interaction, 'pet.commands.use.cooldown.title_md', {
					time: cooldown.time,
				}),
				{
					color: 'Red',
		...
		const components = await simpleContainer(
			interaction,
			(await t(interaction, 'pet.commands.use.success.title_md')) +
				'\n' +
				(await t(interaction, 'pet.commands.use.success.desc', {
